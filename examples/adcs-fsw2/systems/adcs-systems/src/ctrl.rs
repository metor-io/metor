...
//!
//! The port set is the `execute` signature, matching the pack's other
//! function-authored cyclic entries.
    ...
    pub(crate) fn execute(
        ...
        if self.law == Some(ModeCmd::LAW_DETUMBLE) {
            let dipole_b = match &mag_b {
                Some(b) => clamp_dipole(
                    detumble_dipole(self.k_detumble, &e.omega_b, b),
                    MTQ_MAX_DIPOLE,
                ),
                None => V3::zeros(),
            };
        ...
        let torque_b = V3::from_buf(
            torque_b
                .into_buf()
                .map(|t| t.clamp(-RW_TORQUE_MAX, RW_TORQUE_MAX)),
        );
