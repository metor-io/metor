...
| `adcs-systems` | `systems/adcs-systems/` | The three cyclic systems as **one pack in one cdylib** (`fn pack()` + `export_pack!`): the orbiting rigid-body **plant** and its physics (wheel dynamics, disturbance-torque model, GPS/magnetometer error models, magnetorquers, sensor suite), the MEKF **nav** filter (`sun_from_css` reconstruction, its own sun/WMM references at the GPS position, magnetometer-only through eclipses), and the Yang-LQR + magnetorquer-law **ctrl** controller. All three are function-authored from a state struct plus typed init and execute functions. |
