...
/// ExternalWaker is used by `LocalScheduler` to allow external
/// threads to wake
pub trait ExternalWaker: 'static + core::fmt::Debug {
    /// Called when a future is woken by an external thread.
    ///
    /// Typically this will wake the currently sleeping reactor
    fn wake(&self);
}

impl ExternalWaker for () {
    fn wake(&self) {}
}
...
feature! {
    ...
    #[derive(Clone, Debug)]
    pub struct LocalSpawner(Weak<Core>, Weak<dyn ExternalWaker>);
    ...
    impl LocalScheduler {
        ...
        /// Creates a new LocalScheduler with the specified reactor
        pub fn with_external_waker(waker: impl ExternalWaker) -> Self {
            Self {
    ...
    impl LocalSpawner {
        ...
        where
            ...
            F::Output: Send + 'static,
        {
            match (self.0.upgrade(), self.1.upgrade())  {
                (Some(core), Some(waker))  => LocalScheduler { core, waker, _not_send: PhantomData }.spawn(future),
                _ => JoinHandle::error(task::join_handle::JoinErrorKind::Shutdown),
            }
        }
        ...
        {
            match (self.0.upgrade(), self.1.upgrade()) {
                (Some(core), Some(waker)) => LocalScheduler { core, waker, _not_send: PhantomData }.spawn_allocated(task),
                _ => JoinHandle::error(task::join_handle::JoinErrorKind::Shutdown),
            }
        }
    }
