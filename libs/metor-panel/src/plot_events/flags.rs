...
pub(crate) const GUTTER_H: f32 = 16.0;
...
pub(crate) const FLAG_HIT_PX: f32 = 8.0;
...
pub(crate) fn mix(bg: Hsla, fg: Hsla, t: f32) -> Hsla {
    let b = Rgba::from(bg);
    let f = Rgba::from(fg);
...
pub(crate) fn cluster_events(positioned: Vec<(Pixels, PlotEvent)>) -> Vec<EventCluster> {
...
pub(crate) fn paint_event_flags(
    pb: Bounds<Pixels>,
    ...
    for (i, cluster) in clusters.iter().enumerate() {
        ...
        if x < pb.origin.x || x > right {
            hits.push(Bounds::new(pb.origin, gpui::size(px(0.0), px(0.0))));
            continue;
        }
