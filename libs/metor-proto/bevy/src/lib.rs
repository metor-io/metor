...
use metor_proto::types::RequestId;
use metor_proto::{
...
use metor_proto_bbq::{AsyncArcQueueRx, RxExt};
use metor_proto_wkt::{
...
pub use metor_proto_bbq::PacketGrantR;
pub use metor_proto_wkt::ComponentValue;
pub use metor_proto_wkt::ElementValueMut;
pub use metor_proto_wkt::{ComponentPart, ComponentPath};
...
) -> Result<(), metor_proto::error::Error> {
    let mut count = 0;
...
where
    C: metor_proto::component::Component + Decomponentize + Default + Component,
{
    fn insert(
        &self,
        commands: &mut Commands,
...
impl Plugin for DefaultAdaptersPlugin {
    fn build(&self, app: &mut bevy::prelude::App) {
        ...
        app.add_impeller_component::<metor_proto_wkt::SimulationTimeStep>();
    }
}
...
impl Plugin for Impeller2Plugin {
    fn build(&self, app: &mut bevy::prelude::App) {
        app.add_plugins(DefaultAdaptersPlugin)
            .insert_resource(metor_proto_wkt::SimulationTimeStep(0.001))
            .insert_resource(metor_proto_wkt::CurrentTimestamp(Timestamp::EPOCH))
            .insert_resource(metor_proto_wkt::LastUpdated(Timestamp::now()))
            .insert_resource(metor_proto_wkt::EarliestTimestamp(Timestamp::now()))
    ...
    [
        Stream {
            behavior: StreamBehavior::FixedRate(FixedRateBehavior {
                initial_timestamp: metor_proto_wkt::InitialTimestamp::Earliest,
