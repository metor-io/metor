...
fn instantiate(wasm: &[u8], fuel: u64) -> (Store<()>, Instance) {
...
fn call_f64(wasm: &[u8], name: &str, args: &[f64]) -> f64 {
    let (mut store, instance) = instantiate(wasm, 1_000_000);
    let func = instance.get_func(&store, name).expect("export is present");
    let params: Vec<wasmi::Val> = args.iter().map(|a| wasmi::Val::F64((*a).into())).collect();
    let mut out = [wasmi::Val::F64(0.0.into())];
    func.call(&mut store, &params, &mut out).unwrap();
    match &out[0] {
...
fn defined_functions(wasm: &[u8]) -> usize {
    wasmparser::Parser::new(0)
        .parse_all(wasm)
        .filter_map(|p| match p.unwrap() {
